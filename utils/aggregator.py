...
class Aggregator:
    ...
    PPS_DATE_COL = 'pps_sample_start_date' # the date column of the PPS df (this is created in the PpsTextFileProcessor)
    ...
    def merge_pps_quag_on_station_rosette_time(self, quag_df: pd.DataFrame):
        """
        ...
        pps_df_sorted = self.pps_df.sort_values(
            [self.PPS_DATE_COL, self.PPS_STATION_ID_COL, self.PPS_EVENT_NUM_COL]
        )
        ...
        pps_df_sorted[self.PPS_DATE_COL] = pd.to_datetime(pps_df_sorted[self.PPS_DATE_COL])
